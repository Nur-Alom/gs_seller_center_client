...
const Customers = () => {
    ...
    const deleteCustomer = (id) => {
        fetch(`https://gs-seller-center-server.up.railway.app/delete-user/${id}`, {
            method: "DELETE",
        })
            .then(res => res.json())
            .then(data => {
    ...
    const toastSuccess = () => toast.success("Customers Information Delete Successfully!!");
        ...
        <div className='px-6 mx-auto'>
            ...
            <div className='bg-white border border-gray-300 rounded-md'>
                <form onSubmit={handleSubmit(onSubmit)} className='flex justify-between items-center px-4 py-6'>
                    <input {...register("search")} className='w-full focus:bg-white bg-gray-100 p-3 border border-gray-300 outline-0 rounded-md text-sm font-sans font-semibold' type="search" placeholder='Search by name/email/phone' />
                    {search && <svg className="ml-2 mr-4 h-6 w-6 animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
